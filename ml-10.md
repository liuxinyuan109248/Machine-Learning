...
<span style="color: #6FA8FF">**Clustering**</span> is the task of grouping data points such that points in the same group are more similar to each other than to those in other groups. Common clustering algorithms include <span style="color: #6FA8FF">**K-Means Clustering**</span> and <span style="color: #6FA8FF">**Spectral Graph Clustering**</span>.

### <span style="color: #6ED3C5">K-Means Clustering</span>

Given a set of data points $\{x_1, x_2, \ldots, x_n\}$ in $R^d$, the goal of <span style="color: #6FA8FF">**K-Means clustering**</span> is to partition the data points into $k$ clusters $C_1, C_2, \ldots, C_k$ such that the within-cluster variance $\sum_{i=1}^{k} \sum_{x \in C_i} ||x - \mu_i||^2$ is minimized, where $\mu_i$ is the centroid of cluster $C_i$.
...
<span style="color: #6FA8FF">**Spectral clustering**</span> uses the eigenvalues (spectrum) of similarity matrices to perform dimensionality reduction before clustering in fewer dimensions. It is particularly effective for identifying clusters with non-convex shapes.

To perform spectral clustering, we first represent the data as a graph $G = (V, E)$, where $V$ are the data points. The similarity between points can be modeled in three common ways:

- <span style="color: #6FA8FF">**$\epsilon$-Neighborhood Graph:**</span> Points $x_i$ and $x_j$ are connected if the distance $\|x_i - x_j\| < \epsilon$.
- <span style="color: #6FA8FF">**$k$-Nearest Neighbor (k-NN) Graph:**</span> Point $x_i$ is connected to $x_j$ if $x_j$ is among the $k$ nearest neighbors of $x_i$.
- <span style="color: #6FA8FF">**Fully Connected Graph:**</span> All pairs are connected, typically using a similarity kernel (e.g., Gaussian/RBF) to assign weights.

Once the graph is built, we define the following matrices to analyze its structure:

- <span style="color: #6FA8FF">**Adjacency Matrix ($A$):**</span> Represents the edge weights between vertices: $A_{ij} = w_{i,j}$
- <span style="color: #6FA8FF">**Degree Matrix ($D$):**</span> A diagonal matrix where each entry represents the sum of weights of all edges connected to that vertex: $D_{ii} = \sum_{j=1}^n A_{ij}$
- <span style="color: #6FA8FF">**Unnormalized Graph Laplacian ($L$):**</span> Captures how "smooth" a function is over the graph: $L = D - A$
...
$L$ is symmetric and positive semi-definite because for any vector $f \in R^n$, $f^T L f = \dfrac{1}{2} \sum_{i,j} w_{i,j} (f_i - f_j)^2 \geq 0$.

If $G$ has exactly one connected component, then $L$ has an eigenvalue 0 with eigenvector $\mathbf{1}$ (the all-ones vector). If $f$ is an eigenvector corresponding to the eigenvalue 0, then $0 = f^T L f = \dfrac{1}{2} \sum_{i,j} w_{i,j} (f_i - f_j)^2$, which implies that $f_i = f_j$ for all connected nodes $i$ and $j$. Since the graph is connected, $f$ must be a constant vector, i.e., $f = c \mathbf{1}$ for some constant $c$. Thus, the eigenspace corresponding to the eigenvalue 0 is spanned by $\mathbf{1}$.

If $G$ has $k$ connected components $A_1, A_2, \ldots, A_k$, then the Laplacian matrix $L$ can be block-diagonalized into $k$ blocks corresponding to each connected component. Each block has an eigenvalue 0 with eigenvector being the indicator vector of that component. Therefore, the multiplicity of the eigenvalue 0 is $k$, and the eigenspace is spanned by the indicator vectors $I_{A_1}, I_{A_2}, \ldots, I_{A_k}$.
