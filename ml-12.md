...
<span style="color: #6FA8FF">**BPDA Attack on JPEG Compression:**</span> The core challenge of attacking a model protected by <span style="color: #6FA8FF">**JPEG compression**</span> is that the quantization step in JPEG creates a "staircase" function where the gradient is zero almost everywhere. BPDA bypasses this by using a surrogate gradient. The victim model can be represented as a composition of two functions:
...
1.  **Backward Propagation (The Trick)**:
...
2.  **Update Image:** Update pixels using the sign of the approximate gradient: $x_{adv} = x + \alpha \cdot \text{sign}(\nabla_x L)$.
3.  **Iterate**: Repeat until the image $x_{adv}$ successfully misleads the classifier even after being processed by the real JPEG algorithm.
